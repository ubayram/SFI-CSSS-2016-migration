...
% Last Modified by GUIDE v2.5 28-Jun-2016 18:22:03
...
calculateVelocity(springData, handles.bird_ids);
...
calculateVelocity(fallData, handles.bird_ids);
...
%             %Working on legend, doesn't quite work yet
%             bird_ids = [bird_ids unique(birds_year_i)];
%             bird_ids_c = mat2cell(bird_ids, ones(1,length(bird_ids)));
%             bird_ids_c = cellfun(@(x) num2str(x), bird_ids_c, 'UniformOutput', false);
%             legend(bird_ids_c);
%             set(handles.axes1, 'zticklabel', {handles.date_(index_)});     
            hold on;
        end
...
function y2006_DeleteFcn(hObject, eventdata, handles)
% hObject    handle to y2006 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
